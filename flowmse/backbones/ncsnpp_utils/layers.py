...
def variance_scaling(
    scale, mode, distribution, in_axis=1, out_axis=0, dtype=torch.float32, device=None
):
    """Ported from JAX."""
...
def ddpm_conv1x1(in_planes, out_planes, stride=1, bias=True, init_scale=1.0, padding=0):
    ...
    weight_init = default_init(init_scale)(conv.weight.data.shape)
    conv.weight.data.copy_(weight_init.to(conv.weight.device))
...
def ddpm_conv3x3(
    ...
    weight_init = default_init(init_scale)(conv.weight.data.shape)
    conv.weight.data.copy_(weight_init)
    nn.init.zeros_(conv.bias)
    return conv
...
class ResnetBlockDDPM(nn.Module):
    ...
    def __init__(
        ...
        if temb_dim is not None:
            ...
            weight_init = default_init()(self.Dense_0.weight.data.shape)
            self.Dense_0.weight.data.copy_(weight_init.to(self.Dense_0.weight.device))
