...
if __name__ == '__main__':
    ...
    model.eval()
    model._swap_to_ema()  # Use EMA weights for inference
    model.cuda()
    import re
    ...
    for cnt, noisy_file in tqdm(enumerate(noisy_files)):
        ...
        # Load wav
        x_np, x_sr = read(join(clean_dir, filename))
        x = torch.from_numpy(x_np).float().unsqueeze(0)
        if x_sr != sr:
            import torchaudio
            x = torchaudio.functional.resample(x, x_sr, sr)
        y_np, y_sr = read(noisy_file)
        y = torch.from_numpy(y_np).float().unsqueeze(0)
        if y_sr != sr:
            import torchaudio
            y = torchaudio.functional.resample(y, y_sr, sr)
